...
void init_pdm_sender(int pin_dat, int pin_clk) {
    ...
    for(size_t i = 0; i < sizeof(buffer_tx)/4; i++) {
        ...
        buffer_tx[i]= 	0x0103070F; // 1, 2, 3, 4 bits
        // buffer_tx[i]= (1<<(i % 32));
	...
	printf("%4d: %08x\t = %d (actual value)\n", i, buffer_tx[i],__builtin_popcount(buffer_tx[i]));
        if (i & 1) {
		uint64_t v = *((uint64_t *)&(buffer_tx[i-1]));
		printf("\t%016llx\t = %d (actual value)\n\t",v,__builtin_popcountll(v));
		for(int j =0; j < 16; j++)
			printf("%d", (__builtin_popcountll((v>>(4*(15-j))) & 0xF)));
		printf("\t = %d # (Q=4)\n\t", __builtin_popcount(buffer_tx[i-1]) +  __builtin_popcount(buffer_tx[i]));
		for(int j =0; j < 8; j++)
			printf("%d ", (__builtin_popcountll((v>>(8*(7-j))) & 0xFF)));
		printf("\t = %d # (Q=8)\n", __builtin_popcount(buffer_tx[i-1]) +  __builtin_popcount(buffer_tx[i]));
	};
    };
